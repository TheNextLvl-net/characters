...
final class CharacterTagBillboardCommand extends SimpleCommand {
    private CharacterTagBillboardCommand(final CharacterPlugin plugin) {
        super(plugin, "billboard", null);
    }

    public static LiteralArgumentBuilder<CommandSourceStack> set(final CharacterPlugin plugin) {
        final var command = new CharacterTagBillboardCommand(plugin);
    ...
    public int run(final CommandContext<CommandSourceStack> context) {
        ...
        final var billboard = tryGetArgument(context, "billboard", Billboard.class).orElse(Billboard.CENTER);
        final var success = character.getTagOptions().setBillboard(billboard);
        final var message = success ? "character.tag.billboard" : "nothing.changed";
