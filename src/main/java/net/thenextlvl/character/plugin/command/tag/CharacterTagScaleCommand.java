...
final class CharacterTagScaleCommand extends SimpleCommand {
    private CharacterTagScaleCommand(final CharacterPlugin plugin) {
        super(plugin, "scale", null);
    }

    public static LiteralArgumentBuilder<CommandSourceStack> set(final CharacterPlugin plugin) {
        final var command = new CharacterTagScaleCommand(plugin);
    ...
    public int run(final CommandContext<CommandSourceStack> context) {
        ...
        final var scale = tryGetArgument(context, "scale", float.class).orElse(1.0f);
        final var success = character.getTagOptions().setScale(scale);
        final var message = success ? "character.tag.scale" : "nothing.changed";
