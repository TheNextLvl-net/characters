...
final class CharacterTagLineWidthCommand extends SimpleCommand {
    private CharacterTagLineWidthCommand(final CharacterPlugin plugin) {
        super(plugin, "line-width", null);
    }

    public static LiteralArgumentBuilder<CommandSourceStack> set(final CharacterPlugin plugin) {
        final var command = new CharacterTagLineWidthCommand(plugin);
    ...
    public int run(final CommandContext<CommandSourceStack> context) {
        ...
        final var width = tryGetArgument(context, "width", int.class).orElse(200);
        final var success = character.getTagOptions().setLineWidth(width);
        final var message = success ? "character.tag.line-width" : "nothing.changed";
